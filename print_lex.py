...
parser = argparse.ArgumentParser(
    description="Print an entire lexicon or a single term's definitions to the command line.",
)
parser.add_argument(
    "lex",
    help="the lexicon file that should be used (pass a valid JSON filename).",
)
parser.add_argument(
    "-q",
    "--query",
    nargs=1,
    help="allows you to enter a query to search. Type the term you are searching for.",
)
parser.add_argument(
    "-m",
    "--method",
...
if args.query is None:
    for i in range(len(lx.lexicon["terms"])):
        fl = fl + lx.get_formatted_term(
            i,
            "index",
            temp=template_list[args.template[0]],
        )
elif args.method is None or args.method[0].lower() == "term-part":
